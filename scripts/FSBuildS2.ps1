...
# 5. Set Permissions on share
$Acl = Get-Acl "C:\WebShare"
$AccessRule = New-Object system.security.accesscontrol.filesystemaccessrule("Everyone", "ReadAndExecute, Synchronize", "ContainerInherit, ObjectInherit", "InheritOnly", "Allow")
$Acl.SetAccessRule($AccessRule)
Set-Acl "C:\WebShare" $Acl

# 6. Create network share
Net Share WebShare=C:\WebShare "/grant:Everyone,READ"

# 7. Add additional local Admin accounts
$userList = @{}
...
# if $User3 and $Pass3 are not null, then add to the hash $userList
if (-not ([string]::IsNullOrEmpty($User3) -or [string]::IsNullOrEmpty($Pass3) )) {
  $userList.Add($User3, $Pass3)
}
